...
void initLed(void) {
    PCC->CLKCFG[PCC_PORTB_INDEX] |= PCC_CLKCFG_CGC(1); //cau hinh clock cho PORTB
    PORTB->PCR[4] = PORT_PCR_MUX(1); // cau hinh thanh ghi cho PORTB
    FGPIOB->PDDR |= GREEN_LED_PIN; //set cai thanh ghi PDDR nay la Green led (bit thu 4)
    FGPIOB->PDOR |= GREEN_LED_PIN; //set cai thanh ghi PDDR nay la Green led (bit thu 4)
}

void initButton(void) {
    PCC->CLKCFG[PCC_PORTD_INDEX] |= PCC_CLKCFG_CGC(1); //76 tuc la bit mau xanh
